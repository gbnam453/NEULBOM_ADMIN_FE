        ...
        <div className="max-w-xl mx-auto p-6 bg-gray-50 min-h-screen">
            ...
            <div className="fixed top-0 left-0 right-0 w-full bg-white shadow-md z-50">
                <div className="max-w-xl mx-auto flex justify-center items-center p-4 relative">
                    ...
                    <button onClick={fetchDownloads} className="absolute right-4 icon-button">
            ...
            <div className="pt-20">
                ...
                <ul className="mt-6 space-y-3">
                    {downloads.map((download) => (
                        <li key={download.id} className="flex items-center bg-white p-4 rounded-lg shadow-sm relative">
                            {/* 지역 표시 (색상 적용) - 기존 타입 위치에 지역을 표시 */}
                            <span
                            ...
                            <div className="ml-4 flex flex-col flex-grow overflow-hidden">
                                <span className="font-semibold text-lg truncate">{download.title}</span>
                                <span className="text-gray-500 text-sm truncate">{download.category} | {download.type}</span>
                            </div>

                            {/* 우측: 더보기 버튼 */}
                            <div className="relative">
                                <button onClick={() => toggleMenu(download.id)} className="icon-button">
